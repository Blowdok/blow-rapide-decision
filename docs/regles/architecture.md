...
- `src/main/` : processus principal d'Electron. `index.ts` (fenêtre, menu, choix du trousseau), `ipc.ts` (canaux vérifiés), `service.ts` (corpus ouvert, triages, copies rangées, caches, journal des envois ; sans Electron), `rangement.ts` (copie sûre et bilan CSV), `stockage.ts` (réglages, clés chiffrées), `trousseau-linux.ts` (libsecret sous Hyprland).
...
- `src/renderer/` : interface React en français, pensée pour un débutant ([RAD 0006](decisions/0006-interface-pour-debutant.md)).
  - Écrans : Documents (accueil en trois étapes, aperçu et correction des destinations, copie rangée et bilan), Recherche, Comparaison, Réglages (essentiel d'abord, réglages avancés repliés) et Aide. Ils restent montés et seul l'écran affiché est visible : une lecture de dossier, une comparaison ou des réglages en cours survivent à un changement d'écran.
...
Après le triage, « Préparer une copie rangée » affiche le moteur actif et la destination de chaque document. Les destinations peuvent être corrigées; les cas incertains vont par défaut dans « À vérifier ». Après confirmation, le processus principal copie les documents sous un nouveau dossier et ajoute le bilan CSV; il ne déplace ni ne remplace les sources. La copie n’appelle aucun moteur.
...
- Les canaux IPC n'acceptent que le cadre principal de la fenêtre de l'application et vérifient leurs paramètres. Seuls les fichiers du dossier indexé peuvent être ouverts ou copiés; le rangement valide les chemins, les catégories, l’état des fichiers et refuse liens symboliques, destination interne à la source et écrasement.
- Clés API chiffrées par le système, jamais renvoyées à l'interface. Sous Linux/Hyprland, Electron est configuré pour utiliser `gnome-libsecret`; si `safeStorage` ne confirme pas un backend chiffré, l’enregistrement reste refusé.
...
- `npm run test:e2e` : parcours de l'application réelle, puis parcours des options face à un faux serveur Ollama; nécessite un affichage X/Wayland. Sur le poste BV-LIHA, Xwayland écoute sur `:0`; en environnement sans écran, utiliser un serveur virtuel installé (par exemple `xvfb-run -a`).
