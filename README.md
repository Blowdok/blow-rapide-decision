...
L'interface s'adresse à un débutant ([RAD 0006](docs/regles/decisions/0006-interface-pour-debutant.md)) :
...
**Options facultatives** (tous les modes) : les cocher dans Réglages, « Options locales facultatives », puis cliquer sur « Actualiser » dans l'écran Documents. Modèles par défaut, s'ils ne sont pas déjà installés :
...
1. Documents : suivre l'accueil en trois étapes, « Choisir un dossier… », puis « Classer les documents ».
...
2. `npm run dev`, mode « Référence sans IA », dossier `jeux-evaluation/demo/documents`, « Classer les 12 documents » : chaque ligne reçoit une catégorie.
...
6. Options : cocher la recherche par le sens, enregistrer, puis « Actualiser » dans Documents (« Recherche par le sens prête » s'affiche). Chercher « rémunération » : `fiche-paie-aout-2026.txt` devrait remonter, avec la mention « Trouvé par le sens ». Relancer la Comparaison : le rapport ajoute la colonne « Lexical et sémantique ».
7. Lecture des PDF scannés : scanner une page (ou imprimer un PDF en image), la déposer dans le dossier, cocher l'option, « Actualiser » ; le document porte la pastille « OCR » et « Texte lu par l'agent » montre la transcription.
8. Débutant : quitter Ollama, puis relancer l'application en mode Local. L'accueil affiche « À faire », « Comment faire ? » ouvre l'aide « Installer Ollama », et « Essayer sans IA en attendant » passe au mode sans IA.
